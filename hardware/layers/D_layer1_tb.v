...
module D_layer1_tb;
    // Inputs: 784 elements (from image or generator output)
    reg signed [15:0] inputs [0:783];
    reg signed [16*784-1:0] flat_input;
    reg clk;
    reg rst;
    reg start;

    // Outputs: 256 elements
    wire signed [16*256-1:0] flat_output;
    wire done;

    // Instantiate Unit Under Test (UUT)
    layer1_discriminator_v2 uut (
        .clk(clk),
        .rst(rst),
        .start(start),
    ...
    integer i_pack;
    always @(*) begin
        for (i_pack = 0; i_pack < 784; i_pack = i_pack + 1) begin
    ...
    initial begin
        ...
        $display("   784 inputs -> 256 neurons");
        $display("--------------------------------------------------");
        ...
        for (m = 0; m < 20; m = m + 1) begin
            $display("[%2d] = %f (hex: %h)", 
                     m, 
                     q6_10_to_real($signed(flat_output[(m+1)*16-1 -: 16])),
